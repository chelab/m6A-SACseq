...
rule map_to_contamination_by_bowtie2:
    ...
    shell:
        ...
        """
        {params.path_bowtie2} -p {threads} --no-unal --end-to-end --fast --un-conc {params.un} -x {params.ref_bowtie2} -1 {input.r1} -2 {input.r2} > {output.sam} 2>{output.report}
        """
...
rule map_to_spike_by_bowtie2:
    ...
    shell:
        """
        {params.path_bowtie2} -p {threads} --no-unal --end-to-end -L 16 -N 1 --mp 5 --un-conc {params.un} -x {params.ref_bowtie2} -1 {input.r1} -2 {input.r2} > {output.sam} 2>{output.report}
        """
...
rule blastn_to_bam:
    ...
    shell:
        """
        {params.path_blast2bam} {input.xml} ../ref/spike-in_with_N.fa {input.fq} | \
            samtools calmd -@ {threads} --input-fmt-option 'filter=pos < 10 && pos + qlen > 33 && !flag.unmap' --output-fmt BAM - ../ref/spike-in_with_N.fa 2>/dev/null > {output}
        """
